...
logging.basicConfig(filename='memory_profile.log', level=logging.INFO,
...
def profile_decorator(func):
    def wrapper(*args, **kwargs):
        mprof = profile(func, stream=open('memory_profile.log', 'a+'))
        return mprof(*args, **kwargs)
    return wrapper
...
def _evaluate(
    ...
    """
    Evaluates model. Helper function for train_val_test.
    ...
    if is_rank_zero:
        print(f"Evaluation of {stage} set completed.")
...
def test_only(
    ...
    while(1):
        ...
        # test_auroc = _evaluate(args.limit_test_batches, pipeline, test_dataloader, "test")
        # results.test_auroc = test_auroc
        _evaluate(args.limit_test_batches, pipeline, test_dataloader, "test")
